//
// Created by Sidhin Thomas on 19/03/17.
//
/**
 ...
 */
static int
head_bytes (glfs_fd_t *fd, struct stat *statbuf)
{
    int ret;
 ...
 */
static int
head_lines (glfs_fd_t *fd, struct stat *statbuf)
{
    int ret = 0;
    int newline_count = 0;
    ssize_t num_read;
    ...
    finished:
    ret = offset;//glfs_lseek (fd, offset, SEEK_SET);
    //if (ret == -1) {
    //    error (0, errno, "seek error");
    //}
